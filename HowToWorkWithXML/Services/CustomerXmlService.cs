...
public class CustomerXmlService
{
    ...
    {
        this.DocCust = XElement.Load(xmlFilePath);
        this.SalesOrderHeaders = XElement.Load(salesDocFilePath);
        this.SalesOrderDetailsFile = XElement.Load(salesOrderDetailsFile);
    }
    ...
    public IEnumerable<Customer> Find(Func<XElement, bool> filter)
    {
        ...
        if (customersFiltered is null && !customersFiltered.Any()) return Enumerable.Empty<Customer>();
    ...
    public string Join()
    {
        var content = DocCust.Elements("Customer").Join(
            ...
            .Where(saledAndCustomers => saledAndCustomers.Customer.Element("CustomerID").Value == saledAndCustomers.Sales.Element("CustomerID").Value)
    ...
    {
        return SalesOrderDetailsFile.Elements("OrderDetail").Select(x => x.Element("LineTotal").GetAs<decimal>()).Average();
    }
    #endregion
}
